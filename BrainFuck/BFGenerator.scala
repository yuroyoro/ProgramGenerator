...
class BFRuntime( val op:List[Op],val source:String, val size:Int, val maxLoop:Int ){
  val offset = size / 2
...
class BFParser(
  ...
  def parse( s:String ):Option[BFRuntime] = parseAll( brainfuck , s ) match {
    case Success( op, _ )  =>  Some( new BFRuntime( op, s, size, maxLoop) )
...
object Main {
  def main( args:Array[String] ) = {
    ...
    // Hello World
    println( "brainf*ck" )
    bfParser.run(">++++++ほげほげ+++[<+++++ほげほげ+++>-]<.>+++++++[<++++>-]<+.+++++++..+++.[-]>++++++++[<++++>-]<.>++++++ほげほげ+++++[<+++++>-]<.>++++++++[<+++>-]<.+++.------.--------.[-]>++++++++[<++++>-]<+.[-]+++++ほげほげ+++++.")
    bfParser.run(">++++++++++[<++++++++++>-]<++++.---.>++[<+++>-]<+..+++.>++++[>++++++++<-]>.<++++[<++>-]<.>++++[<-->-]<.+++.>++[<--->-]<.>++[<---->-]<.")
    bfParser.run("+++++++++[>++++++++>+++++++++++>+++++<<<-]>.>++.+++++++..+++.>-.------------.<++++++++.--------.+++.------.--------.>+.")

    // Error
    try{ bfParser.run(">+[>]") }catch{ case BFException(msg) => println( msg ) }
    try{ bfParser.run("<+[<]") }catch{ case BFException(msg) => println( msg ) }
    try{ bfParser.run(">+[]") }catch{ case BFException(msg) => println( msg ) }
